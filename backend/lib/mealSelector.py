...
from lib import nutriScore as NutriScore
...
class MealSelector:
    ...
    def select_meals(self, meals_scores, num_meals_to_generate, default_meals):
        ...
        while len(selected_meals_ids) < num_meals_to_generate and len(meals_scores) > 0:
            ...
            probabilities = {
                meal_id: score / total_score for meal_id, score in meals_scores.items()}
            ...
            found_tracked_ingredient = utils.find_elements_in_array(
                meal_ingredients, tracked_ingredients)
            # logger.info("Found tracked Ingredient:")
            # logger.info(found_tracked_ingredient)
            is_rejected = False

            if found_tracked_ingredient:
                is_rejected = reject_tracked_ingredient(
                    found_tracked_ingredient, selected_meals_ingredient)
            if not is_rejected:
                # Add the meal ID to the selected meals if the nutriscore condition is met
                selected_meals_ids.append(meal_id)
                logger.info("Append. Selected meal id:{}".format(
                    len(selected_meals_ids)))
                selected_meals_ingredient = utils.append_without_duplicates(
                    selected_meals_ingredient, found_tracked_ingredient)
...
def reject_tracked_ingredient(tracked_ingredient, selected_meals_ingredients):
    found_elements = utils.find_elements_in_array(
        tracked_ingredient, selected_meals_ingredients)
    if (found_elements):
        return True
    else:
        return False
